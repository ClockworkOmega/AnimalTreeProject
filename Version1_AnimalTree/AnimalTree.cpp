...
Elem& AnimalTree::parentVal(const Elem& searchVal)
{
	Node* tmp = searchNodeString(searchVal, rootVal());
	tmp = tmp->parent;
	return tmp->value;
}
...
bool AnimalTree::isLeaf(const Elem& searchVal)
{
	...
	if (current_->child.size() == 0)		// Determine if the current node
...
bool AnimalTree::isSibling(const Elem& searchVal, const Elem& sibling)
{
	Node* tmp;
	
	tmp = searchNodeString(searchVal, rootVal());
	tmp = tmp->parent;

	for (int i = 0; i < tmp->child.size(); ++i)
	{
		Node* tmpertmp = tmp->child[i];
		if (tmpertmp->value == sibling)
			return true;
	}
...
Node* AnimalTree::searchNodeInt(const int num, Node* subtree)
{
	if (subtree->IDNum == num)
	{
		//cout << "The " << val << " node was found in the tree." << endl;	
		return subtree;
	}
	...
	else
	{
		for (int i = 0; i < subtree->child.size(); ++i)
		{
			Node* result = searchNodeInt(num, subtree->child[i]);
			if (result != NULL)
				return result;
		}
		return NULL;
...
// Set the current node to another
// node using its ID number
void AnimalTree::searchInt(const int num)
{
	Node* tmp;
	tmp = searchNodeInt(num, rootVal());
...
Elem AnimalTree::checkSiblings(const Elem& searchVal)
{
	...
	if (isRoot(searchVal) == true)									// If the node is the root
	{
		return noSiblingsLeft;										// Then return no siblings
	}
	else
	{
		...
		{															// If that sibling has already been checked
			Node* tmpertmp = tmp->child[i];									// use that sibling.
			if (tmpertmp->check == false)
				return tmpertmp->value;
		}
